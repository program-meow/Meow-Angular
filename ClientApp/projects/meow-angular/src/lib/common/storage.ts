...
export class Storage {
  ...
  setLocalItem(key: string, value, expires: number = 360000) {
    if (this.meow.helper.isEmpty(key))
      return;
    this.removeLocalItem(key);
    let content = new StorageWrapper(value, Date.now(), expires * 1000);
    let json = this.meow.helper.toJson(content);
  ...
  getLocalItem<T>(key: string): T {
    ...
    if (this.meow.helper.isEmpty(json))
      return null;
    if (!this.meow.helper.isJson(json))
      return <T>json;
    let content = this.meow.helper.toJsonObject<StorageWrapper>(json);
    if (!content)
      return null;
